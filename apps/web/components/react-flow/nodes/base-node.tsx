...
export const BaseNode = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'relative rounded-md border bg-card text-card-foreground ring-primary',
        'hover:ring-1',
        ...
        '[.react-flow\\_\\_node.selected_&]:ring-1!',
        className,
      )}
      tabIndex={0}
      {...props}
...
export const BaseNodeDescription = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      data-slot="base-node-description"
      className={cn('absolute -bottom-8 text-sm font-medium text-center', className)}
      {...props}
    />
  ),
)
BaseNodeDescription.displayName = 'BaseNodeDescription'
