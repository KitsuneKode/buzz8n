...
describe('Redis Migrate Script', () => {
  ...
  test('should handle error when stream already exists', async () => {
    ...
    await expect(
      (client as any).xGroupCreate('workflow:execution', 'workflow:executors', '$', {
        MKSTREAM: true,
      }),
  ...
  test('should quit Redis connection after migration', async () => {
    ...
    const client = createClient()
    await (client as any).on('error', () => {}).connect()
    await (client as any).xGroupCreate('workflow:execution', 'workflow:executors', '$', {
      MKSTREAM: true,
    })
  ...
  test('should handle connection errors', async () => {
    ...
    await expect((client as any).on('error', () => {}).connect()).rejects.toThrow(
      'Connection refused',
    )
  })
})
